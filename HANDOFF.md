...
**Current phase:** Phase 4 COMPLETE → starting Phase 5 (live market data). Monorepo live on GitHub `origin/main` at commit `77621d0`. Real per-user `holdings`/`transactions`/`watchlist` tables (owner-only RLS) live; Dashboard/Portfolio/Watchlist read real data with empty states; watchlist add/remove writes to Supabase; current-price lookups isolated behind the single stub `app/src/lib/marketData/index.ts` (the Phase 5 swap point).
