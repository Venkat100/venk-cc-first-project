...
- **Where server-side logic lives — RESOLVED (2026-06-16): TanStack Start server functions.** Both market-data fetching (Phase 5) and trade execution (Phase 6) run as TanStack Start server functions — same codebase, shared TS types, one deploy, less setup than standing up Supabase Edge Functions (no Deno runtime, no separate function deploy/secrets pipeline). The market-data API key lives in a server-only env var. Supabase remains the database + auth + RLS layer. This honors the hard rules: API key hidden server-side, browser never calls the provider, trades validated server-side against a server-fetched price. (Edge Functions remain a fallback option if we ever need always-on, host-independent endpoints.)
