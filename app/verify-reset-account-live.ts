...
async function main() {
  ...
  const { uid: uidA } = await step("create rich test user (A)", 15000, () => createTestUser(admin, emailA, PASSWORD));
  const { uid: uidB } = await step("create isolation test user (B)", 15000, () => createTestUser(admin, emailB, PASSWORD));
