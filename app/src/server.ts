...
async function normalizeCatastrophicSsrResponse(response: Response, pathname: string): Promise<Response> {
  ...
  captureServerError(recovered, { route: pathname });
...
export default {
  async fetch(request: Request, env: unknown, ctx: unknown) {
    try {
      ...
      // the same reason as those — keep server-only code off the SSR path.
      if (pathname === "/api/health") {
        const { handleHealthRequest } = await import("./lib/health/endpoint.server");
      ...
      console.error(error);
      captureServerError(error, { route: new URL(request.url).pathname });
      return new Response(renderErrorPage(), {
        status: 500,
        headers: { "content-type": "text/html; charset=utf-8" },
      });
