...
export type BriefItem = {
  symbol: string;
  one_line_what_happened: string;
  why_it_matters: string;
  /** The real Finnhub article this item is grounded in — attached
   *  DETERMINISTICALLY IN CODE from the same news payload already fetched
   *  for generation, never asked of or generated by Claude (which would
   *  fabricate a plausible-looking URL). Absent when no matching article
   *  had a URL, or on any brief row generated before this field existed —
   *  both cases render as "no link", never a guess. */
  article_url?: string;
  article_source?: string;
  /** true for a broad-market fallback item (e.g. SPY/QQQ), added only to
   *  pad out a thin brief — false/absent for the user's own holdings or
   *  watchlist symbols. Set in code from symbol membership, not trusted
   *  from Claude's output, so the label can never be wrong even if the
   *  model's own judgment about what to include drifts. */
