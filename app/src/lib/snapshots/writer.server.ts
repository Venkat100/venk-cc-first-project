...
// holdings_value + options_value (O3: live Black-Scholes value of any option
// positions — see getOptionsValueByUser below; holdings_value itself stays
// equities-only, its established meaning elsewhere), then UPSERT today's
...
export async function runSnapshots(opts: { onlyUserId?: string } = {}): Promise<SnapshotSummary> {
  ...
  // Options (O3): ONE batched fetch for every user's options book (same
