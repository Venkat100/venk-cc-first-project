...
## Queued epic — Options & Margin paper trading (after the Robinhood-parity R1–R3 epic ships)
Venky's ask (2026-07-23): let users try options trading and margin risk-free — a separate "Options & Margin" section. Educational core: experience time decay, expiration, leverage, and margin calls with virtual money.
- **Options v1 scope:** long calls/puts + sell-to-close only (no spreads, no naked shorts). Chains GENERATED by us; contracts priced via **Black-Scholes** from live underlying price + volatility (free data tiers don't include real chains — premiums are model-derived, disclosed in the UI). Real exercise/expiry mechanics; educational explainers.
- **Margin v1 scope:** opt-in margin mode — borrow up to ~2× buying power, daily simulated interest, maintenance requirement, and simulated MARGIN CALLS (warn → forced-liquidation simulation). Same server-side tamper-proof engine patterns.
- Sequence when started: data/pricing module → DB model → options UI (chain + order + positions) → margin engine → margin UI → hardening. Design details TBD at kickoff.
