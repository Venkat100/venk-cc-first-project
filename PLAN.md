...
| # | Item | Why here |
|---|---|---|
| **1** | **A1 — Account management** + starting capital $100k→$25k | P0. Today a locked-out user is locked out forever; open signup is irresponsible without password reset. Capital change rides along because it touches signup. |
| **2** | **D1 — Durable Postgres price cache** | Pulled FORWARD from Phase D. Free, and the single biggest capacity unlock (~10–50×). Decouples user count from the provider rate limit entirely: N users watching AAPL = 1 provider call, not N. Must exist before growth, not after. |
| **3** | **Live prices + live intraday chart** | The visible payoff of #2 — a static price feels dead in a trading app. ~15–30s refresh, chart extends through the day, tick flash, polls only during market hours. Nearly free ON TOP of #2; prohibitively expensive without it. (True tick-level WebSocket streaming is a later upgrade: Finnhub's free tier includes it, but Vercel serverless can't hold persistent connections → needs a ~$5/mo always-on relay, and the key can't go in the browser.) |
| **4** | **Options discoverability** | Venky couldn't find options at all — they're only a tab on Stock Detail. A feature nobody can find may as well not exist. Add an Options nav entry / global view (positions + symbol picker to browse chains), and surface from Markets. Small. |
| **5** | **A2/A3/A4 — abuse guards, ops visibility, legal surface** | Per-user rate limits on AI endpoints, error monitoring (Sentry free), analytics, Terms/Privacy/disclaimers. **All must land before open public signup.** |
