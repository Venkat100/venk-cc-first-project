...
## 6d. WATCHLIST — `verify-insights.ts` / `verify-journal.ts` live-quote flakiness — OPEN, opened 2026-08-17

Both flaked in the same full-suite run (2026-08-17), within ~7 seconds of each other, both on a live NVDA quote fetch: `verify-insights.ts` threw `No live data for NVDA` out of `getStockInsight`'s `providerQuotes([sym])` call (`insights.server.ts:129`); `verify-journal.ts`'s real NVDA quote fetch took ~4.5s (unusually slow) then the immediately-following `execute_trade` buy failed `invalid_price`. Both scripts were re-run standalone immediately after and passed cleanly with no code changes — consistent with the `verify-hardening-pass.ts` precedent (§6c): ordinary transient network variance, not a regression. The suspected mechanism: both scripts, along with several others in the 35-script suite, hit the real Finnhub/Twelve Data providers back-to-back with no coordination between scripts, and this pair happened to land on NVDA within the same few-second window as other scripts' own live NVDA/VOO calls (`verify-hardening-valuation.ts`, `verify-eventstudy-live.ts` immediately precede them in suite order) — plausible transient rate-limit/timeout pressure on the shared free-tier provider quota, not a bug in either script or in this session's own code changes (neither script's failure path touches anything this session modified — `insights.server.ts:129` still checks `price > 0`, unchanged by the `Quote.ok` follow-up).

**Trigger, matching the §6c precedent exactly: if either flakes TWO more times, fix it structurally rather than continuing to re-run.** Recorded here now so the count isn't lost across sessions.

**Read on the likely real fix, if the trigger fires**: this looks different from §6c's `verify-hardening-pass.ts` case, which was fixed by isolating scenarios that were internally slow (real DB/RPC chains). Here the suspect is *provider-side* pressure from 35 scripts sharing one free-tier rate limit with no pacing between them. The three candidate fixes, roughly in order of preference: (1) **stagger/rate-limit-aware backoff in the shared test harness** (`verify-harness.ts`) — a small retry-with-backoff wrapper around live provider calls used by scripts that need a REAL quote as setup (as opposed to scripts that are actually testing the quote layer itself), since a provider hiccup during setup isn't the thing being tested; (2) **stagger the suite runner** so live-quote-heavy scripts don't cluster back-to-back — cheaper to try first, no code change to the scripts themselves, just the `for f in verify-*.ts` loop's ordering/spacing; (3) **stub the quote** in scripts where the live fetch is incidental setup, not the subject under test — `verify-journal.ts` needs *a* real trade to attach a journal note to, not specifically NVDA's live price, so it's a better stubbing candidate than `verify-insights.ts`, which is deliberately testing the real Claude+live-data insight pipeline and should probably keep a real fetch even if that means keeping the retry-wrapper instead. Not implementing any of these now — the trigger hasn't fired.
